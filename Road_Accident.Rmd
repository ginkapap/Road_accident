...
confusionMatrix(regreg_pred,twenty_observed)
```
#### 5-fold test
```{r warning=FALSE}
...
confusionMatrix(regreg_pred,dftemp_test$accident_severity)
```
#### 5-fold test
```{r warning=FALSE}
results_regreg5 = train(accident_severity~., 
...
confusionMatrix(preds_regreg, twenty_observed)
...
## logistic GAM (generalized additive model)
...
confusionMatrix(preds_knn, twenty_observed)
```
#### 5-fold test
```{r}
knn_opts = data.frame(k=c(seq(3, 11, 2)))
...
confusionMatrix(preds_svm, twenty_observed)
```
#### 5-fold test
```{r warning=FALSE}
